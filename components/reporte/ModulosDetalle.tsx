...
function RecItem({ rec }: { rec: Rec }) {
  ...
  const textColor = { ok: 'text-gray-700', warn: 'text-gray-700', error: 'text-gray-700' }
  return (
    <li className="flex items-start gap-2">
      {icons[rec.type]}
      <span className={`text-sm leading-snug ${textColor[rec.type]}`}>{rec.text}</span>
    </li>
  )
}
...
function recsP2c(business: Business): Rec[] {
  ...
  if (reviews < 20) {
    recs.push({ type: 'error', text: 'Muy pocas reseñas — aumenta el volumen con mensajes de seguimiento post-estadía.' })
  } else if (reviews < 50) {
    recs.push({ type: 'warn', text: `${reviews} reseñas. Implementa un proceso sistemático: QR en habitaciones, tarjeta en el check-out, mensaje por WhatsApp.` })
  }
...
function recsP2d(socialData: Record<string, string | null>): { platform: SocialPlatform; presente: boolean }[] {
  return SOCIAL_PLATFORMS.map(p => ({
    platform: p,
    presente: !!socialData[p.key],
  }))
}
...
function recsP2e(report: Report): Rec[] {
  ...
  if (googleAi) {
    recs.push({ type: 'ok', text: 'Tu negocio aparece en el AI Overview de Google — máxima visibilidad en búsquedas.' })
  } else {
    recs.push({ type: 'error', text: 'No apareces en el AI Overview de Google. Para mejorar: publica contenido de autoridad en tu sitio (blog de viajes, guías locales) y obtén menciones en directorios relevantes.' })
  }

  if (perplexity) {
    recs.push({ type: 'ok', text: 'Tu negocio es mencionado por Perplexity AI.' })
  } else {
    recs.push({ type: 'warn', text: 'No apareces en Perplexity AI. Regístrate en directorios de autoridad: TripAdvisor, Sernatur, Lonely Planet, Booking y similares.' })
  }

  if (structured) {
    recs.push({ type: 'ok', text: 'Tu sitio web tiene datos estructurados (JSON-LD) — ayuda a los motores de IA a entender tu negocio.' })
  } else {
    recs.push({ type: 'error', text: 'Tu sitio web no tiene datos estructurados (JSON-LD). Agrega Schema.org para Lodging/Hotel: nombre, dirección, calificación, precio.' })
  }

  if (guides && guides > 0) {
    recs.push({ type: 'ok', text: `Mencionado en ${guides} guía${guides > 1 ? 's' : ''} de autoridad — excelente para SEO y visibilidad en IA.` })
  } else {
    recs.push({ type: 'warn', text: 'No se detectaron menciones en guías de viaje de autoridad. Contacta a bloggers de viaje y plataformas como Lonely Planet o Sernatur para que te incluyan.' })
  }
...
function recsP2f(platformData: Record<string, string | null>): Rec[] {
  ...
  if (present.length === 0) {
    recs.push({ type: 'error', text: 'No tienes presencia en ninguna OTA. Booking.com y Airbnb son prioritarios — llegan a millones de viajeros.' })
  } else {
    recs.push({ type: 'ok', text: `Presencia en ${present.length} plataforma${present.length > 1 ? 's' : ''}: ${present.map(p => p.name).join(', ')}.` })
  }
  ...
  present.forEach(p => {
    ...
    if (rating > 0 && rating < 8.0) {
      recs.push({ type: 'warn', text: `${p.name}: calificación de ${rating}/10 — responde a reseñas negativas y actualiza fotos y descripción.` })
    } else if (rating >= 8.0) {
      recs.push({ type: 'ok', text: `${p.name}: excelente puntuación (${rating}/10) con ${reviews} reseñas.` })
    }
  })

  missing.forEach(p => {
    recs.push({ type: 'warn', text: `No estás en ${p.name}. Considera registrarte para ampliar tu alcance.` })
  })
...
function ModuleCard({ index, code, label, desc, max, score, nota, children }: ModuleCardProps) {
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <div className="h-2.5 rounded-full bg-gray-100 overflow-hidden mt-2">
              <motion.div
                ...
                transition={{ delay: index * 0.06 + 0.15, duration: 0.65, ease: 'easeOut' }}
          ...
          <div className="flex items-center gap-3 shrink-0">
            <span className={`text-xl font-bold tabular-nums ${scoreColor(pct)}`}>
              {score}
              <span className="text-gray-300 font-normal text-sm">/{max}</span>
            </span>
            {open
...
export default function ModulosDetalle({ report, business, socialData, platformData }: Props) {
  const modules = [
    {
      code: 'P2a', label: 'Google Maps',    desc: 'Ficha y reseñas',    max: 20, key: 'score_p2a' as const,
      nota: report.modulo_p2a?.nota ?? null,
    },
    {
      code: 'P2b', label: 'Sitio web',      desc: 'Presencia propia',   max: 20, key: 'score_p2b' as const,
      nota: report.modulo_p2b?.nota ?? null,
    },
    {
      code: 'P2c', label: 'Reputación',     desc: 'Calificaciones',     max: 20, key: 'score_p2c' as const,
      nota: report.modulo_p2c?.nota ?? null,
    },
    {
      code: 'P2d', label: 'Redes sociales', desc: 'Instagram y más',    max: 15, key: 'score_p2d' as const,
      nota: report.modulo_p2d?.nota ?? null,
    },
    {
      code: 'P2e', label: 'IA & SEO',       desc: 'Posicionamiento',    max: 5,  key: 'score_p2e' as const,
      nota: report.modulo_p2e?.nota ?? null,
    },
    {
      code: 'P2f', label: 'Plataformas',    desc: 'Booking, Airbnb…',   max: 20, key: 'score_p2f' as const,
      nota: report.modulo_p2f?.nota ?? null,
    },
  ]
      ...
      {modules.map((m, i) => {
        ...
        return (
          ...
          >
            ...
            {/* P2d — per-platform social breakdown */}
            {m.code === 'P2d' && (
